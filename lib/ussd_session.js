...
var createSession = function() {
    ...
    ussd_session.responseCompleted = function() {
        ...
        if(this.modem.ussd_pdu && (args[2] == '15' || args[2] == '0' || args[2] == '1'))
		...
		this.emit('response', args[0], response.replace(/\u0000/g, '').replace(/@*$/, ''), args[2]);
    ...
    ussd_session.encode = function(string) {
        if(ussd_session.modem.ussd_pdu)
			return pdu.encode7Bit(pdu.messageToNumberArray({text: string, encoding: '7bit'}));
        else
            return string;
    }
